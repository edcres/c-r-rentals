...
class Repository(private val db: RentsRoomDatabase) {
    ...
    @WorkerThread
    suspend fun insertRental(rentedItem: RentedItem): Long = db.rentedItemsDao().insert(rentedItem)

    @WorkerThread
    suspend fun updateRental(rentedItem: RentedItem) = db.rentedItemsDao().update(rentedItem)
    ...
    @WorkerThread
    suspend fun deleteRental(rentedItem: RentedItem) = db.rentedItemsDao().delete(rentedItem)
}